  ...
  if (rectangle.width < rectangle.height && rectangle.height <= fixedWidth) {
    ...
    for (int x = 0; x <= fixedWidth - rectangle.height; x++) {
      ...
      for (const Rectangle &r : rectangles) {
        if (x < r.position.x + r.width + r.position.r * (r.height - r.width) &&
            x + rectangle.height > r.position.x &&
            rectangle.height <= fixedWidth - x) {
          y = max(y, r.position.y + r.height +
                         r.position.r * (r.width - r.height));
        }
      }
    ...
    // Asumiendo que todas las instancias van a ser factibles, lo coloca sin
    // rotar
    for (int x = 0; x <= fixedWidth - rectangle.width; x++) {
      int y = 0;
      for (const Rectangle &r : rectangles) {
        if (x < r.position.x + r.width + r.position.r * (r.height - r.width) &&
            x + rectangle.width > r.position.x &&
            rectangle.width <= fixedWidth - x) {
          y = max(y, r.position.y + r.height +
                         r.position.r * (r.width - r.height));
        }
      }
...
void Strip::printOutput() {
  ...
  cout << currentHeight << "\n" << totalUnsedArea << endl;
  for (const Rectangle &rectangle : rectangles) {
    cout << rectangle.position.x << " " << rectangle.position.y << " "
         << rectangle.position.r << endl;
  }
}
